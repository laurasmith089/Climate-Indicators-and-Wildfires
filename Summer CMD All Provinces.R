...
# Initialize an empty data frame to store all summer CMD data
all_summer_cmd_data <- data.frame()
...
for (file_path in file_list) {
  ...
  data <- data[c("Year", "AG_SUMMERAVGCMD")]
  ...
  all_summer_cmd_data <- rbind(all_summer_cmd_data, data)
}

# Prepare for external plotting
par(mar = c(5, 4, 4, 2) + 0.1, oma = c(0, 0, 0, 15), xpd = NA)

# Plotting all Summer CMD data together
plot(NULL, xlim = range(all_summer_cmd_data$Year), ylim = range(all_summer_cmd_data$AG_SUMMERAVGCMD), 
     xlab = "Year", ylab = "Summer Climatic Moisture Deficit (mm)", 
     main = "Summer Climatic Moisture Deficit Across Jurisdictions")
colors <- rainbow(n = length(unique(all_summer_cmd_data$Province)))

# Loop to add lines for each province
for(i in unique(all_summer_cmd_data$Province)) {
  province_data <- all_summer_cmd_data[all_summer_cmd_data$Province == i,]
  lines(province_data$Year, province_data$AG_SUMMERAVGCMD, type = "l", col = colors[which(unique(all_summer_cmd_data$Province) == i)])
}

# Add a legend outside the plot to the right
legend(x = par("usr")[2] + 0.5, y = par("usr")[4], legend = unique(all_summer_cmd_data$Province), col = colors, lty = 1, title = "Jurisdictions", horiz = FALSE, xpd = NA)
